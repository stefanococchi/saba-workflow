...
# ── Mapping step types → possible events ──────────────────────────
# Base events that always apply to any workflow
_BASE_EVENTS = ['workflow_started', 'status_changed']

# Events generated by specific step types
_STEP_TYPE_EVENTS = {
    'email':                ['email_scheduled', 'email_sent', 'email_delivered', 'email_opened',
                             'email_clicked', 'email_failed', 'email_completed', 'email_skipped'],
    'sms':                  ['email_scheduled', 'email_sent', 'email_failed', 'email_completed', 'email_skipped'],
    'whatsapp':             ['email_scheduled', 'email_sent', 'email_failed', 'email_completed', 'email_skipped'],
    'webhook':              ['email_completed', 'email_failed'],
    'wait_until':           ['email_completed', 'email_skipped'],
    'goal_check':           ['goal_met', 'goal_not_met'],
    'export_data':          ['email_completed', 'email_failed'],
    'condition':            ['condition_evaluated'],
    'survey':               ['landing_opened', 'form_submitted', 'survey_submitted',
                             'email_sent', 'email_completed'],
    'human_approval':       ['landing_opened', 'email_sent', 'email_completed',
                             'approval_granted', 'approval_rejected'],
    'excel_write':          ['email_completed', 'email_failed'],
    'document_processing':  ['email_completed', 'email_failed'],
    'document_check':       ['landing_opened', 'form_submitted', 'email_sent', 'email_completed',
                             'document_check_validated', 'document_check_rejected'],
    'sister_visura':        ['email_completed', 'email_failed'],
    'sister_ipotecaria':    ['email_completed', 'email_failed'],
    'verifica_report':      ['email_completed', 'email_failed'],
}

# Events from landing page features (detected from step config)
_LANDING_EVENTS = ['landing_opened', 'form_submitted', 'unsubscribed']
_PAYMENT_EVENTS = ['payment_initiated', 'payment_completed', 'payment_cancelled',
                   'payment_expired', 'payment_refunded', 'payment_disputed', 'payment_failed']


def compute_available_events(steps_data):
    """Compute available event types from a list of step dicts."""
    events = set(_BASE_EVENTS)
    for step in steps_data:
        stype = step.get('type', 'email')
        if isinstance(stype, str):
            type_val = stype
        else:
            type_val = stype.value if hasattr(stype, 'value') else str(stype)

        # Add events for this step type
        for evt in _STEP_TYPE_EVENTS.get(type_val, []):
            events.add(evt)

        # Steps with email always have landing-related events
        if type_val in ('email', 'sms', 'whatsapp'):
            lpc = step.get('landing_page_config') or step.get('skip_conditions') or {}
            if lpc.get('fields') or lpc.get('landing_template_id'):
                events.update(_LANDING_EVENTS)

        # Payment events if payment is enabled
        sc = step.get('skip_conditions') or {}
        if sc.get('payment_enabled'):
            events.update(_PAYMENT_EVENTS)

    return sorted(events)


def _update_available_events(workflow, steps_data):
    """Recompute and save available_events in workflow.config."""
    available = compute_available_events(steps_data)
    config = dict(workflow.config) if workflow.config else {}
    config['available_events'] = available
...
def create_workflow():
    ...
    try:
        ...
        if 'steps' in data:
            ...
            # Pre-compute available events for this workflow
            _update_available_events(workflow, data['steps'])
